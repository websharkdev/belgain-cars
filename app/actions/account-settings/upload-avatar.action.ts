...
export const uploadAvatarAction = async (formData: FormData): Promise<ActionResult> => {
    const limit = await checkRateLimit({ key: "upload-avatar", windowInSeconds: 3600, maxRequests: 10 });
    if (!limit.success) {
    ...
    if (!ALLOWED_TYPES.includes(file.type)) return { error: "Only JPG, PNG and WEBP images are allowed." };
    if (file.size > MAX_BYTES) return { error: "File exceeds the 2 MB limit." };

    const headerList = await headers();
    const session = await auth.api.getSession({ headers: headerList });
    if (!session) return { error: "You must be signed in to update your avatar." };
