...
export const deleteAccountAction = async (
    ...
    const limit = await checkRateLimit({ key: "delete-account", windowInSeconds: 3600, maxRequests: 3 });

    if (!limit.success) {
        return { error: "Too many delete account attempts. Please try again later." };
    }
    ...
    try {
        ...
        if (!session) {
            return { error: "You need to sign in again before deleting your account." };
        }
        ...
        if (!parsed.success) {
            return { error: parsed.error.issues[0]?.message ?? "Invalid delete account confirmation." };
        }
