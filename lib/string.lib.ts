...
export function getFullName(
  ...
  return (
    [compactWhitespace(firstName), compactWhitespace(lastName)]
      .filter(Boolean)
      .join(' ') ||
    compactWhitespace(fallback) ||
...
export function getCompactName(
  ...
  return (
    normalizedFirstName || compactWhitespace(fallback) || DEFAULT_USER_FALLBACK
  );
}
...
export function getUserInitials({
  ...
  const fullName = [compactWhitespace(firstName), compactWhitespace(lastName)]
    ...
    .join(' ');
  const fallbackName =
    compactWhitespace(name) || getEmailName(email) || fallback;
...
export function getUserDisplayName({
  ...
  const fallbackName =
    compactWhitespace(name) || compactWhitespace(email) || fallback;
